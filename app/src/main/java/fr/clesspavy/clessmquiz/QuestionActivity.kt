...
class QuestionActivity : AppCompatActivity() {
        ...
        submit.setOnClickListener {
            if(selecedOption!=0)
            {
                ...
                if(currentPosition==questionList!!.size)
                    submit.text="TERMINER"
                else
                    submit.text="Passer à la suivante"
            }else
            {
                currentPosition++
                when
    ...
    fun setColor(opt: Int,color:Int) {
        when (opt) {
            ...
            5 -> {
                opt_5.background = ContextCompat.getDrawable(this, color)
            }
        }
    }
    ...
    fun setOptionStyle()
    {
        ...
        optionList.add(4,opt_5)

        for(op in optionList)
        {
            op.setTextColor(Color.parseColor("#676060"))
